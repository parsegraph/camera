...
app.get("/", async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>camera</title>`);
  write(`</head>`);
  write(`<body>`);
  write(
    `<h1>camera <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`
  );
  write(
    `<p>This library is available as JavaScript UMD module: <a href='/parsegraph-camera.js'>parsegraph-camera.js</a></p>`
  );
...
app.listen(port, () => {
  console.log(`See camera build information at http://localhost:${port}`);
});
